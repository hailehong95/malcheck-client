...
from malcheck_client.utils import zip_list_file, message_box, form_validate
...
def thread_task(window, progress_bar, report_name):
    try:
        # Start checking
        ...
        # report_name = f"{address_}_{id_}_{name_}"
        obj_path = zip_list_file(report_name)
        progress_bar.update_bar(9, 10)

        obj_name = os.path.basename(obj_path)
        pre_signed_url = get_pre_signed_url(obj_name)
    ...
    while True:
        ...
        if event == 'submit':
            ...
            if form_validate(values):
                ...
                report_name = f"{address_}_{id_}_{name_}"
                window.perform_long_operation(lambda: thread_task(window, progress_bar, report_name), 'thread_task')
            else:
